...
export default function OtpEmailVerification({ code, locale = "es" }: OtpEmailVerificationProps) {
  const t = getEmailTranslations(locale)

  return (
    <EmailLayout preview={t("otpEmailVerification.preview")} locale={locale}>
      <Preview>{t("otpEmailVerification.preview")}</Preview>

      <Text style={{ fontSize: "11px", color: "#a1a1aa", margin: "0 0 8px 0", textTransform: "uppercase", letterSpacing: "0.1em" }}>
        {t("otpEmailVerification.badge")}
      </Text>
      <Text style={{ fontSize: "20px", fontWeight: 600, color: "#18181B", margin: "0 0 8px 0", lineHeight: 1.3 }}>
        {t("otpEmailVerification.heading")}
      </Text>
      <Text style={{ fontSize: "14px", color: "#3f3f46", margin: "0 0 32px 0", lineHeight: 1.6 }}>
        {t("otpEmailVerification.intro")}
      </Text>
      ...
      <Text style={{ fontSize: "13px", color: "#71717A", margin: "0 0 8px 0" }}>
        {t("otpEmailVerification.expiry")}
      </Text>

      <Text style={{ fontSize: "12px", color: "#a1a1aa", margin: 0 }}>
        {t("otpEmailVerification.footer")}
      </Text>
    </EmailLayout>
  )
}
