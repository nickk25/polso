...
import { VaultItemSheet } from "./vault-item-sheet"
...
const SOURCE_KEYS = ["upload", "telegram", "email"] as const
...
export function VaultTable({ items, total, pages, page, statusFilter }: VaultTableProps) {
  ...
  const txForItem = (item: VaultItem) =>
    item.transaction ?? item.matchSuggestion?.transaction ?? null
            ...
            <TableBody>
              {items.length === 0 ? (
                ...
                items.map((item) => {
                  ...
                  const statusKey = STATUS_KEYS.includes(item.status as typeof STATUS_KEYS[number]) ? item.status : "processing"
                  const Icon = STATUS_ICONS[statusKey] ?? Spinner
                  const variant = STATUS_VARIANTS[statusKey] ?? "secondary"
                  const tx = txForItem(item)
                        ...
                        {tx ? (
                          <div className="flex flex-col gap-0.5">
                            <span className="font-medium truncate max-w-[180px]">
                              {tx.merchantName ?? tx.description ?? t("table.transactionFallback")}
                            </span>
